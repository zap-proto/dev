...
Browser → server is **TLS 1.3**. Chrome / Safari handle the KEM (recent versions ship `X25519MLKEM768`), the application runs over `wss://`. The ZAP wire format is fine over TLS, but the X-Wing handshake described in [post-quantum](/docs/post-quantum) terminates at the server, not in the browser. End-to-end PQ ZAP between a browser and an origin requires a server-side terminator; the browser hop is TLS.
...
The Edge Worker (Cloudflare Workers / Deno / Bun — any V8 isolate runtime) acts as the ZAP terminator. The browser sees `fetch`; the Worker holds the persistent X-Wing-authenticated ZAP connection upstream.

A first-class browser/WASM SDK is on the roadmap — see the [SDK status table on /docs/post-quantum](/docs/post-quantum#per-language-status). Today, talk to your origin via a thin Worker-side adapter using [zap-proto/ts](https://github.com/zap-proto/ts) or [zap-proto/zap-es](https://github.com/zap-proto/zap-es); the X-Wing/KEM hop happens server-side.
